...
class OnlineBookStore:
    ...
    cart_menu = ['돌아가기', '장바구니 조회', '수량 변경', '상품 삭제', '주문하기']
    guest_menu = ["종료", "로그인", "회원가입", "도서조회"]
    ...
    def run_cart_menu(self):
        print('>>>>>>> 장바구니 <<<<<<<<')
        while True:
            menu = self.select_menu(self.cart_menu)
            if menu == 0:
                break
            elif menu == 1:
    ...
    def menu_update_cart_count(self):
        ...
        while True:
            try:
                book_no = int(input('>> 도서번호 : '))
                break
            except ValueError:
                print("숫자만 입력해 주세요.")
        while True:
            try:
                count = int(input('>> 변경할 수량 : '))
                break
            except ValueError:
                print('숫자만 입력하세요.')
    ...
    def menu_place_order(self):
        ...
        address = input(f' >> 배송지 입력 (엔터 시 [{default_addr}]) : ') or default_addr
        if not address:
            print('배송지가 필요합니다.')
            return
        #주문 생성
    ...
    def menu_modify_member(self):
        ...
        member.set_email(email)
        member.set_address(address)
        if self.msv.modify_member_profile(id, member):
            print('회원 정보를 수정했습니다.')
        else:
            print('수정에 실패했습니다. ')
    ...
    def menu_view_order_detail(self):
        ...
        while True:
            try:
                order_no = int(input('>> 삭제할 주문번호 : '))
                break  # 정상적으로 숫자가 입력되면 반복문 탈출
            except ValueError:
                print("숫자만 입력해 주세요.")
